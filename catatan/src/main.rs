...
fn create_edit_file(root: &mut Folder) -> io::Result<String> {
    ...
    if root.find_file(&file_name).is_none() {
        println!("creating  a new file with name : {}", file_name);
...
fn create_folder(root: &mut Folder) -> io::Result<String> {
    ...
    if root.find_subfolder(&folder_name).is_none() {
        let new_folder = Folder::new(folder_name.clone());
        root.add_folder(new_folder);

        let folder_path = format!("notes/{}", folder_name);
        fs::create_dir(folder_path)?;
